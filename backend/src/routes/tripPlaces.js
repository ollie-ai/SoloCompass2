...
router.get('/:tripId/places', requireAuth, async (req, res) => {
  try {
    ...
    const places = await db.prepare('SELECT id, trip_id, place_id, name, address, category, notes, visited, created_at FROM trip_places WHERE trip_id = ? ORDER BY created_at DESC').all(tripId);
...
router.post('/:tripId/places', requireAuth, sanitizeAll(['name', 'address', 'notes']), tripPlaceValidation.create, handleValidationErrors, async (req, res) => {
  try {
    const { tripId } = req.params;
    const { name, place_id, address, category, visited, notes } = req.body;
    ...
    const result = await db.prepare(`
      INSERT INTO trip_places (trip_id, user_id, name, place_id, address, category, visited, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(tripId, req.userId, name, place_id || null, address || null, category || 'other', visited || false, notes || null);

    const place = await db.prepare('SELECT id, trip_id, place_id, name, address, category, notes, visited, created_at FROM trip_places WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', requireAuth, sanitizeAll(['name', 'address', 'notes']), tripPlaceValidation.update, handleValidationErrors, async (req, res) => {
  try {
    ...
    if (visited !== undefined) { updates.push('visited = ?'); params.push(visited); }
    ...
    const place = await db.prepare('SELECT id, trip_id, place_id, name, address, category, notes, visited, created_at FROM trip_places WHERE id = ?').get(id);

...
import { 
  ArrowLeft, ArrowRight, Copy, Check, Volume2, RefreshCw,
  Globe, Languages, MessageSquare, Send
} from 'lucide-react';
...
export default function Translator() {
  ...
  const handleTranslate = async (inputText = text) => {
    ...
    try {
      ...
      if (res.data.data?.translations?.[0]?.text) {
        const result = res.data.data.translations[0].text;
        setTranslatedText(result);
        
        setHistory(prev => [
          { original: inputText, translated: result, source: sourceLang, target: targetLang },
          ...prev.slice(0, 9)
        ]);
      }
    } catch (err) {
        ...
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl font-black text-base-content">Quick Translator</h1>
            </div>
            <p className="text-base-content/60 text-sm">
              Translate essential phrases for your travels.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="flex-1">
              ...
              <select
                ...
                className="w-full p-3 bg-base-200 border border-base-300 rounded-xl text-sm font-medium"
              >
                {LANGUAGES.map(lang => (
            ...
            <button
              onClick={handleSwap}
              className="mt-6 p-2 bg-base-200 hover:bg-base-300 rounded-lg transition-colors"
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              className="w-full p-4 bg-base-200 border border-base-300 rounded-xl text-base-content placeholder:text-base-content/40 resize-none"
              rows={3}
            />
          </div>
          ...
          <button
            ...
            className="w-full py-3 bg-brand-vibrant text-white rounded-xl font-bold flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {loading ? (
            ...
            <div className="flex flex-wrap gap-2">
              {QUICK_PHRASES.map((phrase) => (
                <button
                  ...
                  onClick={() => {
                    setText(phrase);
                    setTimeout(() => handleTranslate(phrase), 0);
                  }}
                  className="px-3 py-2 bg-base-200 hover:bg-base-300 rounded-lg text-sm font-medium text-base-content/80 transition-colors"
                >
                  {phrase}
                </button>
              ))}
